...
```bash
PHASE_INFO=$(node ~/.claude/get-stuff-done/bin/gsd-tools.cjs find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/get-stuff-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/get-stuff-done/bin/gsd-tools.cjs find-phase "${PHASE}" --raw)
```
