...
```bash
PHASE_INFO=$(node ~/.claude/get-stuff-done/bin/gsd-tools.js find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/get-stuff-done/bin/gsd-tools.js roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/get-stuff-done/bin/gsd-tools.js find-phase "${PHASE}" --raw)
```
