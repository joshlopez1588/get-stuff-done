...
PHASES=$(node ~/.claude/get-stuff-done/bin/gsd-tools.cjs phases list)
...
```bash
node ~/.claude/get-stuff-done/bin/gsd-tools.cjs commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md
```
