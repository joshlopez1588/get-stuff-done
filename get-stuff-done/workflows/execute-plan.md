...
```bash
INIT=$(node ~/.claude/get-stuff-done/bin/gsd-tools.cjs init execute-phase "${PHASE}" --include state,config)
```
...
See `~/.claude/get-stuff-done/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/get-stuff-done/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/get-stuff-done/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
```bash
node ~/.claude/get-stuff-done/bin/gsd-tools.cjs commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
</step>
...
```bash
node ~/.claude/get-stuff-done/bin/gsd-tools.cjs commit "" --files .planning/codebase/*.md --amend
```
</step>
