...
```bash
INIT=$(node ~/.claude/get-stuff-done/bin/gsd-tools.js init execute-phase "${PHASE}" --include state,config)
```
...
```bash
node ~/.claude/get-stuff-done/bin/gsd-tools.js commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
</step>
...
```bash
node ~/.claude/get-stuff-done/bin/gsd-tools.js commit "" --files .planning/codebase/*.md --amend
```
</step>
