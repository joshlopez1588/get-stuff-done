...
```bash
timestamp=$(node ~/.claude/get-stuff-done/bin/gsd-tools.cjs current-timestamp full --raw)
```
</step>

<step name="commit">
```bash
node ~/.claude/get-stuff-done/bin/gsd-tools.cjs commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/phases/*/.continue-here.md
```
</step>
