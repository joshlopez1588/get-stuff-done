...
```bash
TRANSITION=$(node ~/.claude/get-stuff-done/bin/gsd-tools.js phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node ~/.claude/get-stuff-done/bin/gsd-tools.js progress bar --raw)
```
