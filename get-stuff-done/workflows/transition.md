...
```bash
TRANSITION=$(node ~/.claude/get-stuff-done/bin/gsd-tools.cjs phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node ~/.claude/get-stuff-done/bin/gsd-tools.cjs progress bar --raw)
```
