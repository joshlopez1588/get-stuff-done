...
```bash
ROADMAP=$(node ~/.claude/get-stuff-done/bin/gsd-tools.cjs roadmap analyze)
```
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node ~/.claude/get-stuff-done/bin/gsd-tools.cjs summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
```bash
ARCHIVE=$(node ~/.claude/get-stuff-done/bin/gsd-tools.cjs milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
```bash
INIT=$(node ~/.claude/get-stuff-done/bin/gsd-tools.cjs init execute-phase "1")
```
