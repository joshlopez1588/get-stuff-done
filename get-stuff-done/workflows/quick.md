...
```bash
INIT=$(node ~/.claude/get-stuff-done/bin/gsd-tools.js init quick "$DESCRIPTION")
```
...
```bash
node ~/.claude/get-stuff-done/bin/gsd-tools.js commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${QUICK_DIR}/${next_num}-PLAN.md ${QUICK_DIR}/${next_num}-SUMMARY.md .planning/STATE.md
```
