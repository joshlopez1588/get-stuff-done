...
```bash
slug=$(node ~/.claude/get-stuff-done/bin/gsd-tools.js generate-slug "$title" --raw)
```
...
```bash
node ~/.claude/get-stuff-done/bin/gsd-tools.js commit "docs: capture todo - [title]" --files .planning/todos/pending/[filename] .planning/STATE.md
```
