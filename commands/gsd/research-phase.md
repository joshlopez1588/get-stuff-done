...
```bash
RESEARCHER_MODEL=$(node ~/.claude/get-stuff-done/bin/gsd-tools.cjs resolve-model gsd-phase-researcher --raw)
```

## 1. Validate Phase

```bash
PHASE_INFO=$(node ~/.claude/get-stuff-done/bin/gsd-tools.cjs roadmap get-phase "${phase_number}")
```
