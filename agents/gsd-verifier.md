...
```bash
ARTIFACT_RESULT=$(node ~/.claude/get-stuff-done/bin/gsd-tools.js verify artifacts "$PLAN_PATH")
```
...
```bash
LINKS_RESULT=$(node ~/.claude/get-stuff-done/bin/gsd-tools.js verify key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(node ~/.claude/get-stuff-done/bin/gsd-tools.js summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(node ~/.claude/get-stuff-done/bin/gsd-tools.js verify commits $COMMIT_HASHES)
fi
