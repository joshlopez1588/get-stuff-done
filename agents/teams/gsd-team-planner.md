...
node ~/.claude/get-shit-done/bin/gsd-tools.js analyze-teams --phase "$PHASE_NUM"
```
...
```bash
node ~/.claude/get-shit-done/bin/gsd-tools.js team-matrix
```
...
for fragment in "$phase_dir"/teams/*-TEAM-PLAN.md; do
  VALID=$(node ~/.claude/get-shit-done/bin/gsd-tools.js frontmatter validate "$fragment" --schema team-plan)
  echo "$fragment: $VALID"
done
```
...
```bash
node ~/.claude/get-shit-done/bin/gsd-tools.js commit "docs($PHASE): create team plan fragments" --files "$phase_dir/teams/"
```
</step>
