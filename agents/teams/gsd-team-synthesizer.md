...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  VALID=$(node ~/.claude/get-stuff-done/bin/gsd-tools.cjs frontmatter validate "$plan" --schema plan)
  STRUCTURE=$(node ~/.claude/get-stuff-done/bin/gsd-tools.cjs verify plan-structure "$plan")
...
```bash
node ~/.claude/get-stuff-done/bin/gsd-tools.cjs commit "docs($PHASE): synthesize team plans into unified PLAN.md files" --files "$PHASE_DIR/"*-PLAN.md "$PHASE_DIR/teams/CONTRACTS.md" "$PHASE_DIR/teams/CONFLICTS.md" "$PHASE_DIR/teams/SYNTHESIS.md" .planning/ROADMAP.md
```
</step>
