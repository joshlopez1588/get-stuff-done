...
for plan in "$PHASE_DIR/teams/"*-TEAM-PLAN.md; do
  ...
  if echo "$wave" | grep -q "1"; then
    echo "Wave 1: $plan"
    node ~/.claude/get-shit-done/bin/gsd-tools.js frontmatter validate "$plan" --schema team-plan
  fi
done
```
...
# Commit team status
node ~/.claude/get-shit-done/bin/gsd-tools.js commit "docs($PHASE): team execution complete" --files "$PHASE_DIR/teams/TEAM-STATUS.md"
```
</step>
